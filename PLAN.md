...
- [x] `gr.Blocks` でレイアウトを構築する
- [x] `gr.Chatbot` でチャット表示エリアを配置する
- [x] テキスト入力欄と送信ボタンを配置する
- [x] ストリーミング表示を実装する（`yield` でトークンを逐次更新）
- [x] 出典エリアを配置する（`gr.Accordion` + `gr.Markdown` でページ番号と引用テキストを表示）
- [x] リセットボタンを配置する（会話履歴リストをクリア）
- [x] 会話履歴を Python リストで管理する（セッション内のみ保持）
- [x] アプリ起動時に `indexer.py` の判定ロジックを呼び出す
